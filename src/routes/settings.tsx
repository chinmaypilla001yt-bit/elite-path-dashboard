    ...
    <AppShell>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <GlassCard glow="blue" className="p-6">
          <div className="font-mono text-[10px] uppercase tracking-[0.24em] text-foreground/45">Profile</div>
          <div className="mt-1 text-lg font-semibold text-foreground">Display name</div>
          <p className="mt-2 text-sm text-foreground/60">Shown in the greeting on your dashboard.</p>
          <input
            value={draftName}
            onChange={(e) => setDraftName(e.target.value)}
            placeholder={user?.displayName || "Chinmay"}
            className="mt-4 w-full rounded-lg border border-foreground/10 bg-foreground/[0.03] px-3 py-2 text-sm text-foreground placeholder:text-foreground/30 focus:border-foreground/30 focus:outline-none"
          />
          ...
          <div className="mt-4 flex justify-end gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg border border-foreground/10 bg-foreground/[0.03] px-3 py-1.5 text-xs text-foreground/70 hover:bg-foreground/[0.06] hover:text-foreground disabled:opacity-40"
            >
            ...
            <button
              onClick={handleSave}
              disabled={!dirty || saving}
              className="inline-flex items-center gap-1.5 rounded-lg bg-[image:var(--gradient-cyber)] px-4 py-1.5 text-xs font-semibold text-foreground shadow-[var(--shadow-glow)] transition hover:opacity-90 disabled:opacity-40"
            >
          ...
          <div className="mt-5 rounded-lg border border-foreground/5 bg-foreground/[0.03] p-3">
            <div className="font-mono text-[10px] uppercase tracking-widest text-foreground/45">Account</div>
            <div className="mt-1 text-sm text-foreground">{user?.email}</div>
            <div className="mt-2 flex items-center gap-3 font-mono text-[10px] uppercase tracking-widest text-foreground/50">
            ...
            <button
              onClick={() => signOut()}
              className="mt-4 inline-flex items-center gap-2 rounded-lg border border-foreground/10 bg-foreground/[0.03] px-3 py-1.5 text-xs text-foreground/80 hover:bg-foreground/[0.06] hover:text-foreground"
            >
...
function NotificationSettingsCard() {
  ...
  return (
    <GlassCard glow="purple" className="p-6">
      <div className="font-mono text-[10px] uppercase tracking-[0.24em] text-foreground/45">Notifications</div>
      <div className="mt-1 flex items-center gap-2 text-lg font-semibold text-foreground">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2">
        {items.map((it) => (
          <label
            key={it.key}
            className="flex cursor-pointer items-start gap-3 rounded-lg border border-foreground/5 bg-foreground/[0.02] p-3 hover:bg-foreground/[0.04]"
          >
            <input
              type="checkbox"
            ...
            <div className="min-w-0">
              ...
              <div className="mt-0.5 text-xs text-foreground/50">{it.desc}</div>
            </div>
          </label>
        ))}
      </div>
      ...
      <div className="mt-5">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {settings.reminderTimes.map((t) => (
            <span
              key={t}
              className="inline-flex items-center gap-1.5 rounded-full border border-foreground/10 bg-foreground/[0.04] px-3 py-1 text-xs text-foreground/80"
            >
              {t}
              <button
                onClick={() => void removeTime(t)}
                className="rounded-full p-0.5 text-foreground/50 hover:text-red-300"
                title="Remove"
              >
          ...
          <div className="inline-flex items-center gap-1 rounded-full border border-foreground/10 bg-foreground/[0.03] px-2 py-1">
            <input
              type="time"
              value={newTime}
              onChange={(e) => setNewTime(e.target.value)}
              className="w-24 bg-transparent text-xs text-foreground outline-none [color-scheme:dark]"
            />
            <button
              onClick={addTime}
              className="rounded-full p-1 text-foreground/70 hover:bg-foreground/[0.06] hover:text-foreground"
              title="Add time"
            >

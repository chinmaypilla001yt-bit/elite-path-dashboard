import { createFileRoute } from "@tanstack/react-router";
import { EditableList } from "@/components/ascend/EditableList";

export const Route = createFileRoute("/career/certifications")({
  head: () => ({
    meta: [
      { title: "Certifications · Career Hub · Ascend" },
      { name: "description", content: "Courses and credentials you can put on a resume." },
      { property: "og:title", content: "Certifications · Ascend" },
      { property: "og:description", content: "Courses and credentials you can put on a resume." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: () => (
    <EditableList
      embed
      storageKey="certifications"
      eyebrow="Career"
      title="Certifications"
      description="Courses and credentials you can put on a resume."
      itemLabel="certification"
      glow="emerald"
      fields={[
        { name: "title", label: "Certification", required: true, placeholder: "AWS Cloud Practitioner" },
        { name: "org", label: "Issuer", placeholder: "Amazon Web Services" },
        { name: "period", label: "Date", placeholder: "Mar 2026" },
        { name: "url", label: "Credential URL" },
        { name: "category", label: "Category", type: "category" },
        { name: "description", label: "Notes", type: "textarea" },
      ]}
      renderItem={(r) => (
        <>
          <div className="font-mono text-[10px] uppercase tracking-widest text-white/45">
            {(r.org as string) || "certification"}
          </div>
          <div className="mt-1 text-lg font-semibold text-white">{r.title as string}</div>
          {r.org ? <div className="text-sm text-white/70">{r.org as string}</div> : null}
          {r.url ? (
            <a href={r.url as string} target="_blank" rel="noreferrer" className="mt-1 block truncate text-xs text-[oklch(0.82_0.14_200)] underline">
              {r.url as string}
            </a>
          ) : null}
          {r.description ? <p className="mt-2 whitespace-pre-wrap text-sm text-white/60">{r.description as string}</p> : null}
        </>
      )}
    />
  ),
});
